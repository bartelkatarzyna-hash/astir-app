...
3. **This week's goals card**: remains below the Home action cards. The card header has a persistent icon-only "Edit" action directly beside the title in empty, in-progress, and completed view states. It uses the shared round icon control recipe, a light pencil-line SVG, muted default color, gold hover, visible focus without the hover fill, and the hover label "Edit" without a triangle. Activating it opens the Edit goals modal with the five setup rows. Empty state shows copy "Set up your goals for this week" directly under the card title and all goal tiles as disabled arcs without counts. In-progress state shows "You're doing great, keep it up." under the card title. Completed state shows "You did it. Take a moment to savor it." under the card title. Unselected goals remain visible as disabled placeholder tiles. Do not show a "Same as last week" action. Do not show text header actions such as "Set up" or "Edit." Applications progress comes only from logged applications. Rest progress is automatic for completed past days with no activity, with a user minus override when an automatic rest day should not count. Networking, Prep, and Paperwork are manually tracked by clicking their tiles.
