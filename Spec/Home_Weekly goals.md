...
Note lines are retired. Goal tiles use labels, gauges, and info icons only. Tiles use `--goal-tile-height`, which keeps the visual bottom space below the label at about 16px, with 12px top and side padding and 16px bottom padding. The gauge keeps 16px of space before the label row, so the editable hover surface can keep its existing height, extend below the gauge arc, and still leave breathing room above the label. The label and info icon sit in one full-width centered row, so the combined visible label cluster stays centered when copy or spacing changes. The info icon box is the icon size, not a larger control box, with only the goal title gap between label and icon.
